...
class GraspNode(Node):
	...
	def grasp(self,pose_T):
		...
		request.kin_name = "ik"
		request.roll = 0.0 
		request.pitch  =  1.3962614348313127#夹取效果不佳可调整这个参数 1.04弧度等于60度表示末端目标姿态与桌面的夹角在60度 1.3962614348313127
		request.yaw = 0.0
	...
	def get_ik_respone_callback(self, future):
		try:
			...
			joints[5] = 30
			raw = list(joints)
			joints = [max(0, min(180, int(round(j)))) for j in joints]
			if joints != [int(round(v)) for v in raw]:
				print("CLAMPED", [round(v,1) for v in raw], "->", joints)
	...
	def move(self):
		...
		time.sleep(2.5)
		self.grasp_flag = True
		print("HOLD: cube gripped and lifted - NOT releasing")
